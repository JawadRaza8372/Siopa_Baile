...
let location=useHistory()
    const [incomeDta, setincomeDta] = useState({index:'',docid:'',data:''})

    let fetch=async()=>{
        await db.collection('Order').onSnapshot((snapshot) => {
            setsellerOrder(snapshot.docs.map(doc=>(({id:doc.id,post:doc.data()}))));
...
let updateStatus=(index,docid,data)=>{
   setincomeDta({index:index,docid:docid,data:data})
    db.collection('Order').doc(docid).get().then((doc)=>{
...
let dataswap=async()=>{
if (incomeDta.data){    updatedata[incomeDta.index]=incomeDta.data;
await db.collection('Order').doc(incomeDta.docid).set({data:updatedata})}
}
useEffect(() => {
dataswap()
console.log('done')
...
if(user){
    ...
    {sellerOrder && sellerOrder.map((sorder)=>(
    sorder.post.data.map((sd,index)=>{
        if (sd.seller === user.userId){
    return (<><br/> <SellerOrderedProductsitems key={index} id={index} docid={sorder.id} orderdata={sd} productsData={data} updatfun={updateStatus}/></>)
        }
