...
export const MyCard = ({productName,imgurl,data}) => {
   ...
   <NavLink  className="myButton2" to={`/productDetail/${data.id}`}><MoreIcon/></NavLink>
<br/>
</div> 
        </>
    )
...
export const MyCard3 = ({productName,imgurl,data,category}) => {
   ...
   <img className="MyCardImage" src={`${imgurl}`} alt="productame"/>
            ...
            <div style={{display:"flex"}}>
                <button onClick={()=>setmodelbtn(true)} className="btn btn-outline-primary mr-4"><EditIcon/></button>
                <button onClick={()=> {db.collection('Products').doc(data.id).delete();location.push("/")}} className="btn btn-outline-danger"><DeleteOutlineIcon/></button>
            </div>

<br/>
</div> </div>
<MyModel3 openModel={modelbtn} closeModel={()=>setmodelbtn(false)} data={data} category={category} />
        </>
    )
}
