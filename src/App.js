...
let ftchProducts=async()=>{
 await db.collection('Products').onSnapshot((snapshot) => {
    setProductsData(snapshot.docs.map(doc=>(({id:doc.id,post:doc.data()}))));
...
useEffect(() => {
  db.collection('Category').onSnapshot((snapshot) => {
    if (snapshot) {
      setCategoryData(snapshot.docs.map(doc=>(({id:doc.id,post:doc.data()}))));
    }
})}, [])
...
location.push('/')
    ...
    return(<>
    <Routes loginStatus={userData} products={ProductsData} category={CategoryData} userlogout={logoutfunction}/>
    </>);
  }
